...
(defn adjust-brightness [^Color c factor]
  ;; Slightly brighten/darken a color by factor (-0.5 to +0.5)
  (let [scale (fn [x] (min 255 (max 0 (int (* x factor)))))
        r (.getRed c)
        g (.getGreen c)
        b (.getBlue c)]
...
(defn row-shade [base row total-rows]
  ;; Generate subtle variation based on row index
  (let [factor (+ 0.85 (* 0.3 (/ row (max 1 (dec total-rows)))))] ;; from 0.85 to 1.15
    (adjust-brightness base factor)))
    ...
    (let [panel (proxy [JPanel] []
                  (paintComponent [^Graphics g]
                    ...
                    (let [g2 (.create ^Graphics2D g)
                      ...
                      ;; Draw grid with multi-row shades
