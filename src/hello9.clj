    ...
    (let [panel (proxy [JPanel] []
                  (paintComponent [^Graphics g]
                    ...
                    (let [g2 (.create ^Graphics2D g)
                          ...
                          size (min w h)
                          padding 80            ;; Slightly smaller padding for a bigger circle
                          arc-size (int (- size (* 2 padding)))
                          arc-x (int (/ (- w arc-size) 2))
                          ;; Move circle down (80% height)
                          arc-y (int (+ (/ h 2) 100))
                      ...
                      ;; Centered timer text (upper part of the screen)
                      ...
                      (let [metrics (.getFontMetrics g2)
                            ...
                            text-y (int (- (/ h 2) 100))] ;; move text above circle
                        (.drawString g2 text
