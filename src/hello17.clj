(ns hello17
  (:import [javax.swing JFrame Timer JPanel]
           [java.awt Font Color Graphics Graphics2D RenderingHints GradientPaint Toolkit Point Cursor]
...
;; Pastel colors
(def pomodoro-colors [(Color. 173 216 230)  ;; Pastel Blue
                      (Color. 152 251 152)  ;; Pastel Green
                      (Color. 255 239 150)  ;; Pastel Yellow
                      (Color. 255 182 193)]) ;; Pastel Pink

(def grid-cols 8)

(def fade-duration 500) ;; ms fade-in per tile
...
(defn -main []
  (let [frame (JFrame. "Pomodoro Grid")
        ...
        tile-activation (atom []) ;; store timestamps per tile
        last-x (atom nil)
        last-tap (atom 0)]
                      ...
                      (let [row (int (/ i grid-cols))
                        ...
                        (if (> alpha 0)
                          (let [grad (GradientPaint. x y
                            ...
                            (.setPaint g2 grad)
                            (doto g2
                              (.setComposite (java.awt.AlphaComposite/getInstance
                                               java.awt.AlphaComposite/SRC_OVER (float alpha)))
                              (.fillRoundRect x y (dec tile-size) (dec tile-size) 20 20))))))

                    ;; Reset composite for text
                    (.setComposite g2 (java.awt.AlphaComposite/getInstance java.awt.AlphaComposite/SRC_OVER 1.0))
